...
(function () {
  ...
  function setEQ(type, gain) {
    ...
    if (!isNaN(freq)) {
      state.eq[freq] = gain;
      const A = state.audio;
      if (A.ctx && A.ctx.state !== 'suspended' && A.eqFilters) {
        const idx = EQ_FREQUENCIES.indexOf(freq);
        if (idx !== -1 && A.eqFilters[idx]) {
          A.eqFilters[idx].gain.setTargetAtTime(Math.pow(10, gain / 20), A.ctx.currentTime, 0.1);
        }
      }
      updateEqUI();
    }
    ...
    if (state.audio.ctx && state.audio.eqFilters) {
      EQ_FREQUENCIES.forEach((freq, i) => {
        ...
        if (state.audio.eqFilters[i]) {
          state.audio.eqFilters[i].gain.setTargetAtTime(gainValue, state.audio.ctx.currentTime, 0.1);
        }
      });
    }
  }
