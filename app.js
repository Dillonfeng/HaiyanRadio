...
(function () {
  ...
  let state = {
    ...
    eq: { 60: 0, 80: 0, 100: 0, 200: 0, 500: 0, 1000: 0, 1500: 0, 2500: 0, 5000: 0, 10000: 0, 15000: 0, 17500: 0 },
    currentTab: 'radio',
    counter: {
      seconds: 0,
      intervalId: null
  ...
  function ensureRadioAudioGraph() {
    ...
    if (A.ctx && A.ctx.state !== 'closed' && A.leftAnalyser && A.rightAnalyser && A.analyser) {
      return true;
    }
    ...
    try {
      ...
      A.masterGain.gain.value = 1.0;
      A.merger.connect(A.masterGain);
      A.masterGain.connect(ctx.destination);

      // ========== 12频段EQ均衡器 ==========
      const EQ_FREQUENCIES = [60, 80, 100, 200, 500, 1000, 1500, 2500, 5000, 10000, 15000, 17500];
      A.eqFilters = [];
  ...
  function connectRadioSource(audioEl) {
    const A = state.audio;
    if (!A.ctx || !A.eqFilters.length) return;
    
    if (!A.sources) {
      A.sources = new Map();
    }
    
    if (!A.sources.has(audioEl)) {
      A.sources.set(audioEl, A.ctx.createMediaElementSource(audioEl));
    }
    
    A.source = A.sources.get(audioEl);
    
    let prevNode = A.source;
    A.eqFilters.forEach(filter => {
      try {
        prevNode.disconnect(filter);
      } catch (e) {}
    ...
    try {
      prevNode.disconnect(A.splitter);
    } catch (e) {}
    prevNode.connect(A.splitter);
    
    A.splitter.connect(A.merger, 0, 0);
    A.splitter.connect(A.merger, 1, 1);
  }
  ...
  const els = {
    ...
    toast: $('toast'),
    stationMenuContent: $('stationMenuContent'),
    stationMenuSidebar: $('stationMenuSidebar'),
    stationMenuBtn: $('stationMenuBtn'),
    stationMenuClose: $('stationMenuClose'),
    stage: document.querySelector('.stage'),
    presetPage: $('presetPage'),
    prevPresetBtn: $('prevPresetBtn'),
    nextPresetBtn: $('nextPresetBtn'),
    ...
    presetChannelList: $('presetChannelList'),
    presetModalNumber: $('presetModalNumber'),

    // 输入框模态框
    inputModal: $('inputModal'),
    inputModalTitle: $('inputModalTitle'),
    inputModalHint: $('inputModalHint'),
    inputModalValue: $('inputModalValue'),
    closeInputBtn: $('closeInputBtn'),
    cancelInputBtn: $('cancelInputBtn'),
    confirmInputBtn: $('confirmInputBtn'),

    // 预置位分配模态框
    presetAssignModal: $('presetAssignModal'),
    presetAssignTitle: $('presetAssignTitle'),
    presetAssignHint: $('presetAssignHint'),
    presetAssignGrid: $('presetAssignGrid'),
    closePresetAssignBtn: $('closePresetAssignBtn')
  };
  ...
  function pauseRadio() {
    ...
    const gridEl = els.radioPresets?.querySelector('.preset-grid');
    if (gridEl) {
      gridEl.querySelectorAll('.preset-btn').forEach(btn => {
  ...
  // 读取 CSS 计数器单元高度，保证与响应式缩放同步
  function updateCounterDisplay(seconds) {
    const COUNTER_CELL = Math.round(parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--counter-cell'))) || 18;
    ...
    digitContainers.forEach((digitEl, idx) => {
      ...
      if (newVal < oldVal) {
        ...
        strip.style.transform = `translateY(-${10 * COUNTER_CELL}px)`;
        counterResetTimers[idx] = setTimeout(() => {
          strip.style.transition = 'none';
          strip.style.transform = `translateY(-${newVal * COUNTER_CELL}px)`;
          // 强制 reflow 后恢复过渡
  ...
  function renderRadioPresets() {
    const panelEl = els.radioPresets;
    const gridEl = panelEl.querySelector('.preset-grid');
    const pageEl = els.presetPage;
    if (!gridEl) return;

    const PRESET_COUNT = 100;
    const PAGE_SIZE = 10;
    const currentPage = state.presetPage.radio || 0;
    const totalPages = Math.ceil(PRESET_COUNT / PAGE_SIZE);
    ...
    for (let i = startNum; i < startNum + 9; i++) {
      const btn = createPresetButton(i);
      gridEl.appendChild(btn);
    }

    const prevBtn = document.createElement('button');
    prevBtn.className = 'nav-btn';
    prevBtn.id = 'prevPresetBtn';
    prevBtn.textContent = '‹';
    prevBtn.addEventListener('click', () => {
      if (state.presetPage.radio > 0) {
        state.presetPage.radio--;
        renderRadioPresets();
      }
    });
    gridEl.appendChild(prevBtn);

    const btn10 = createPresetButton(startNum + 9);
    gridEl.appendChild(btn10);

    const nextBtn = document.createElement('button');
    nextBtn.className = 'nav-btn';
    nextBtn.id = 'nextPresetBtn';
    nextBtn.textContent = '›';
    nextBtn.addEventListener('click', () => {
      if (state.presetPage.radio < totalPages - 1) {
        state.presetPage.radio++;
        renderRadioPresets();
      }
    });
  ...
  function createPresetButton(index) {
    const btn = document.createElement('button');
    btn.className = 'preset-btn';
    btn.dataset.preset = index;
    
    const presetData = getPresetData(index);
    btn.textContent = index;
    if (presetData && presetData.id) {
      btn.dataset.channelId = presetData.id;
    } else {
      btn.dataset.channelId = '';
    }

    if (state.current.radio && presetData && state.current.radio === presetData.id) {
  ...
  function getPresetData(index) {
    const presetKey = 'radio_preset_' + index;
  ...
  function savePreset(index, channel) {
    const presetKey = 'radio_preset_' + index;
    localStorage.setItem(presetKey, JSON.stringify(channel));
    renderRadioPresets();
  }

  function handlePresetClick(index) {
    if (state.presetEditing) {
      openPresetEditor(index);
      return;
    }
    const presetData = getPresetData(index);
    if (presetData && presetData.id) {
      selectRadioChannel(presetData.id);
    } else {
      toast('该预置位为空，请先选择一个频道');
    }
  }
  ...
  function openPresetEditor(index) {
    ...
    els.presetModalNumber.textContent = index;
    renderPresetChannelList();
    els.presetModal.classList.add('show');
  }

  function closePresetEditor() {
    els.presetModal.classList.remove('show');
    editingPresetIndex = null;
    els.presetSearchInput.value = '';
  }

  function renderPresetChannelList(filter = '') {
    const listEl = els.presetChannelList;
    if (!listEl) return;

    const channels = getChannelsFor('radio');
    const filtered = channels.filter(ch => 
      ch.name.toLowerCase().includes(filter.toLowerCase()) ||
      ch.frequency.toLowerCase().includes(filter.toLowerCase())
    );

    listEl.innerHTML = '';

    if (filtered.length === 0) {
      listEl.innerHTML = '<div class="empty-hint" style="padding: 20px; text-align: center;">未找到匹配的频道</div>';
      return;
    }

    filtered.forEach(ch => {
      const item = document.createElement('div');
      item.className = 'preset-channel-item';
      item.dataset.id = ch.id;
      item.innerHTML = `
        <span class="preset-channel-name">${ch.name}</span>
        <span class="preset-channel-freq">${ch.frequency || '网络电台'}</span>
      `;
      item.addEventListener('click', () => {
        savePreset(editingPresetIndex, ch);
        closePresetEditor();
        toast(`已将 "${ch.name}" 保存到预置位 ${editingPresetIndex}`);
      });
  ...
  function clearPreset() {
    if (editingPresetIndex !== null) {
      const presetKey = 'radio_preset_' + editingPresetIndex;
      localStorage.removeItem(presetKey);
      renderRadioPresets();
      closePresetEditor();
      toast(`已清除预置位 ${editingPresetIndex}`);
    }
  }

  function togglePresetEditing() {
    state.presetEditing = !state.presetEditing;
    const panelEl = els.radioPresets;
    const editBtn = $('presetEditBtn');
    
    if (state.presetEditing) {
      panelEl.classList.add('editing');
      if (editBtn) editBtn.classList.add('active');
      toast('进入编辑模式：点击预置按钮选择频道');
    } else {
  ...
  function updateRadioSelection() {
    ...
    if (gridEl) {
      ...
      if (state.current.radio) {
        const activeBtn = gridEl.querySelector('.preset-btn[data-channel-id="' + state.current.radio + '"]');
        if (activeBtn) {
    ...
    const menuContent = els.stationMenuContent;
    if (menuContent) {
      menuContent.querySelectorAll('.station-menu-item').forEach(item => {
        item.classList.remove('active');
      });
      if (state.current.radio) {
        const activeItem = menuContent.querySelector('.station-menu-item[data-id="' + state.current.radio + '"]');
        if (activeItem) {
  ...
  function renderStationMenu() {
    ...
    const list = getChannelsFor('radio');
    const tree = buildStationTree(list);

    menuContent.innerHTML = '';

    const currentStation = state.current.radio;

    tree.forEach((region) => {
      const hasActiveStation = region.children.some(s => s.data.id === currentStation);
      const isExpanded = hasActiveStation;

      const groupDiv = document.createElement('div');
      groupDiv.className = 'station-menu-group' + (isExpanded ? ' expanded' : '');
      groupDiv.dataset.region = region.name;

      groupDiv.innerHTML = `
        <span class="station-menu-group-icon">▶</span>
        <span class="station-menu-group-name">${region.name}</span>
        <span class="station-menu-group-count">${region.children.length}</span>
      `;

      menuContent.appendChild(groupDiv);

      const stationsContainer = document.createElement('div');
      stationsContainer.className = 'station-menu-group-items';
      if (!isExpanded) {
        stationsContainer.style.display = 'none';
      }

      region.children.forEach(stationNode => {
        const station = stationNode.data;
        const itemBtn = document.createElement('button');
        itemBtn.className = 'station-menu-item' + (currentStation === station.id ? ' active' : '');
        itemBtn.dataset.id = station.id;
        itemBtn.innerHTML = `
          <span class="station-menu-item-icon">◉</span>
          <span class="station-menu-item-name">${station.name}</span>
        `;
        itemBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          selectRadioChannel(station.id);
        });
        stationsContainer.appendChild(itemBtn);
      });

      menuContent.appendChild(stationsContainer);
    });

    menuContent.querySelectorAll('.station-menu-group').forEach(group => {
      group.addEventListener('click', function() {
        const isExpanded = this.classList.contains('expanded');
        
        menuContent.querySelectorAll('.station-menu-group').forEach(g => {
          g.classList.remove('expanded');
          const items = g.nextElementSibling;
          if (items && items.classList.contains('station-menu-group-items')) {
  ...
  function toggleStationMenu() {
    if (els.stationMenuSidebar) {
      const isOpen = els.stationMenuSidebar.classList.contains('show');
      if (isOpen) {
        hideStationMenu();
      } else {
        showStationMenu();
  ...
  function showStationMenu() {
    if (els.stationMenuSidebar && els.stage) {
      els.stationMenuSidebar.classList.add('show');
      els.stage.classList.add('stage-with-menu');
      renderStationMenu();
    }
  }
  ...
  function hideStationMenu() {
    if (els.stationMenuSidebar && els.stage) {
      els.stationMenuSidebar.classList.remove('show');
      els.stage.classList.remove('stage-with-menu');
    }
  }

  let menuSearchQuery = '';
  let inputModalCallback = null;
  let presetAssignChannelId = null;

  function openInputModal(title, hint, defaultValue, callback) {
    if (!els.inputModal) return;
    els.inputModalTitle.textContent = title;
    els.inputModalHint.textContent = hint || '';
    els.inputModalValue.value = defaultValue || '';
    inputModalCallback = callback;
    els.inputModal.classList.add('show');
    setTimeout(() => els.inputModalValue.focus(), 100);
  }
  ...
  function confirmInputModal() {
    const value = els.inputModalValue.value.trim();
  ...
  function openPresetAssignModal(channelId) {
    if (!els.presetAssignModal) return;
    presetAssignChannelId = channelId;
    const ch = getChannelsFor('radio').find(c => c.id === channelId);
    if (!ch) return;

    els.presetAssignHint.textContent = `将 "${ch.name}" 保存到预置位：`;

    const grid = els.presetAssignGrid;
    grid.innerHTML = '';

    const currentPage = state.presetPage.radio || 0;
    const startNum = currentPage * 10 + 1;

    for (let i = 0; i < 10; i++) {
      const presetIndex = startNum + i;
      const saved = getPresetData(presetIndex);
      const btn = document.createElement('button');
      btn.className = 'preset-assign-btn' + (saved && saved.id ? ' has-preset' : '');
      btn.innerHTML = `
        ${presetIndex}
        <span class="preset-assign-name">${saved && saved.name ? escapeHtml(saved.name) : '空'}</span>
      `;
      btn.addEventListener('click', () => {
        saveChannelToPreset(channelId, presetIndex);
        closePresetAssignModal();
      });
      grid.appendChild(btn);
    }

    els.presetAssignModal.classList.add('show');
  }

  function closePresetAssignModal() {
    if (els.presetAssignModal) {
      els.presetAssignModal.classList.remove('show');
    }
    presetAssignChannelId = null;
  }

  function saveChannelToPreset(channelId, presetIndex) {
    const ch = getChannelsFor('radio').find(c => c.id === channelId);
    if (!ch) return;
    const presetKey = 'radio_preset_' + presetIndex;
    localStorage.setItem(presetKey, JSON.stringify({ id: ch.id, name: ch.name, url: ch.url, frequency: ch.frequency }));
    renderRadioPresets();
    toast(`已保存到预置位 ${presetIndex}`);
  }

  function handleMenuAction(action) {
    const currentId = state.current.radio;
    const currentCh = currentId ? getChannelsFor('radio').find(c => c.id === currentId) : null;

    switch (action) {
      case 'add':
        state.currentTab = 'radio';
        openEditor(null);
        break;

      case 'group':
        openInputModal('◆ 添加分组 ◆', '请输入新分组名称：', '', (name) => {
          if (!name) return;
          const list = getChannelsFor('radio');
          const exists = list.some(c => (c.description || '其它') === name);
          if (exists) {
            toast('该分组已存在', true);
            return;
          }
          ...
          state.channels.radio.push(tempCh);
          saveChannels();
          renderStationMenu();
          renderPresets();
          toast(`已添加分组 "${name}"`);
      ...
      case 'delete':
        if (currentCh) {
          if (confirm(`确定要删除频道 "${currentCh.name}" 吗？`)) {
            state.channels.radio = state.channels.radio.filter(c => c.id !== currentCh.id);
            if (state.current.radio === currentCh.id) {
      ...
      case 'preset':
        if (currentCh) {
          openPresetAssignModal(currentCh.id);
        } else {
          toast('请先选择一个频道', true);
        }
        break;
  ...
  function toggleMenuSearch() {
    ...
    const existingSearch = menuContent.querySelector('.menu-search-bar');
    if (existingSearch) {
    ...
    searchBar.className = 'menu-search-bar';
    searchBar.innerHTML = `
      <input type="text" class="menu-search-input" placeholder="搜索频道..." value="${menuSearchQuery}">
      <button class="menu-search-clear">✕</button>
    `;
    menuContent.insertBefore(searchBar, menuContent.firstChild);

    const input = searchBar.querySelector('.menu-search-input');
    input.focus();
    input.addEventListener('input', (e) => {
      menuSearchQuery = e.target.value;
      filterStationMenu(menuSearchQuery);
    });

    const clearBtn = searchBar.querySelector('.menu-search-clear');
  ...
  function filterStationMenu(query) {
    const menuContent = els.stationMenuContent;
    if (!menuContent) return;

    const groups = menuContent.querySelectorAll('.station-menu-group');
    const groupItems = menuContent.querySelectorAll('.station-menu-group-items');

    if (!query.trim()) {
      groups.forEach(g => g.style.display = '');
      groupItems.forEach(i => i.style.display = '');
      return;
    }

    const q = query.trim().toLowerCase();
    groups.forEach((group, idx) => {
      const items = groupItems[idx];
      if (!items) return;

      const itemEls = items.querySelectorAll('.station-menu-item');
      let hasMatch = false;

      itemEls.forEach(item => {
        const name = item.querySelector('.station-menu-item-name')?.textContent || '';
        const match = name.toLowerCase().includes(q);
        item.style.display = match ? '' : 'none';
        if (match) hasMatch = true;
      });

      const groupName = group.querySelector('.station-menu-group-name')?.textContent || '';
      ...
      if (groupMatch) {
        ...
        itemEls.forEach(item => { item.style.display = ''; });
      } else if (hasMatch) {
        group.style.display = '';
        items.style.display = 'block';
        group.classList.add('expanded');
  ...
  function openPresetQuickAssign(channelId) {
    const ch = getChannelsFor('radio').find(c => c.id === channelId);
    if (!ch) return;

    const presetCount = 10;
    let msg = `将 "${ch.name}" 保存到预置位（1-${presetCount}）：\n\n`;
    for (let i = 1; i <= presetCount; i++) {
      const saved = state.presets.radio[i - 1];
      const savedName = saved ? saved.name : '（空）';
      msg += `  ${i}. ${savedName}\n`;
    }
    msg += `\n请输入目标预置位编号（1-${presetCount}）：`;

    const input = prompt(msg, '1');
    if (input === null) return;

    const idx = parseInt(input, 10) - 1;
    if (isNaN(idx) || idx < 0 || idx >= presetCount) {
      toast('无效的预置位编号', true);
      return;
    }

    state.presets.radio[idx] = { id: ch.id, name: ch.name };
    saveChannels();
    renderPresets();
    toast(`已保存到预置位 ${idx + 1}`);
  }
  ...
  function buildStationTree(list) {
    ...
    tree.sort((a, b) => {
      const idxA = provinceOrder.indexOf(a.name);
      const idxB = provinceOrder.indexOf(b.name);
    ...
    tree.forEach(region => {
      region.children.sort((a, b) => {
        ...
        if (!cityOrder[region.name]) {
          return nameA.localeCompare(nameB, 'zh-CN');
        }

        const cities = cityOrder[region.name];
        let idxA = -1, idxB = -1;

        for (let i = 0; i < cities.length; i++) {
          if (nameA.includes(cities[i])) { idxA = i; break; }
        }
        for (let i = 0; i < cities.length; i++) {
          if (nameB.includes(cities[i])) { idxB = i; break; }
        }
  ...
  function setupVolumeFader() {
    const fader = els.volumeFader;
    if (!fader) return;
    ...
    function updateFader(clientY) {
      ...
      const maxOffset = trackHeight - sliderHeight;
      
      let deltaY = startY - clientY;
      let percent = startValue + deltaY / maxOffset * 100;
      ...
      setVolume('radio', Math.round(percent));
    }

    slider.addEventListener('mousedown', (e) => {
      dragging = true;
      startY = e.clientY;
      startValue = state.volume.radio;
      e.preventDefault();
    });
  ...
  function setEQ(type, gain) {
    ...
    const freq = parseInt(type);
    if (!isNaN(freq)) {
      state.eq[freq] = gain;
      const A = state.audio;
      if (A.ctx && A.eqFilters) {
        const idx = EQ_FREQUENCIES.indexOf(freq);
        if (idx !== -1 && A.eqFilters[idx]) {
          A.eqFilters[idx].gain.setTargetAtTime(gain, A.ctx.currentTime, 0.1);
        }
      }
      updateEqUI();
      saveEqSettings();
  ...
  function updateVolumeFaderUI() {
    ...
    const percent = state.volume.radio;
  ...
  const EQ_PRESETS = {
    ...
    rock: [6, 5, 4, 3, 1, -2, -1, 1, 3, 5, 6, 6],
    pop: [4, 3, 2, 1, 1, 2, 3, 2, 2, 3, 4, 4],
    jazz: [5, 4, 3, 2, 1, 2, 3, 4, 3, 2, 2, 1],
    classical: [-2, -1, 0, 1, 2, 3, 2, 1, 1, 2, 4, 5]
  };

  function setupEqKnobs() {
    const faders = document.querySelectorAll('.fader[data-band]');
    faders.forEach(fader => {
      ...
      const slider = fader.querySelector('.fader-slider');
      const fill = fader.querySelector('.fader-fill');
      const valueEl = fader.parentElement.querySelector('.fader-value');
      ...
      function updateFader(clientY) {
        ...
        let deltaY = startY - clientY;
        let gain = startValue + deltaY / maxOffset * 12;
        gain = Math.max(-6, Math.min(6, gain));
        gain = Math.round(gain);
        
        const percent = ((gain + 6) / 12) * 100;
        const offset = maxOffset - (percent / 100) * maxOffset;
        
        slider.style.top = offset + 'px';
        fill.style.height = percent + '%';
        
        valueEl.textContent = (gain >= 0 ? '+' : '') + gain + 'dB';
        state.eq[band] = gain;
        updateEqAudio();
      }
  ...
  function updateEqUI() {
    EQ_FREQUENCIES.forEach(freq => {
      const fader = document.querySelector('.fader[data-band="' + freq + '"]');
      if (fader) {
        const track = fader.querySelector('.fader-track');
        const slider = fader.querySelector('.fader-slider');
        const fill = fader.querySelector('.fader-fill');
  ...
  function bindEvents() {
    ...
    if (els.stationMenuBtn) {
      els.stationMenuBtn.addEventListener('click', toggleStationMenu);
    }
    if (els.stationMenuClose) {
      els.stationMenuClose.addEventListener('click', hideStationMenu);
    }

    // 侧边栏菜单底部操作按钮
    const menuFooter = document.querySelector('.station-menu-footer');
    if (menuFooter) {
      menuFooter.querySelectorAll('.menu-action-btn').forEach(btn => {
        btn.addEventListener('click', () => {
    ...
    // 预置编辑按钮
    if ($('presetEditBtn')) {
      $('presetEditBtn').addEventListener('click', togglePresetEditing);
    }

    // 预置模态框事件
    if (els.closePresetBtn) {
      els.closePresetBtn.addEventListener('click', closePresetEditor);
    }
    if (els.cancelPresetBtn) {
      els.cancelPresetBtn.addEventListener('click', closePresetEditor);
    }
    if (els.presetModal) {
      els.presetModal.addEventListener('click', (e) => {
        if (e.target === els.presetModal) closePresetEditor();
      });
    }
    if (els.clearPresetBtn) {
      els.clearPresetBtn.addEventListener('click', clearPreset);
    }
    if (els.presetSearchInput) {
      els.presetSearchInput.addEventListener('input', (e) => {
        renderPresetChannelList(e.target.value);
      });
    }
    ...
    if (els.cancelInputBtn) els.cancelInputBtn.addEventListener('click', closeInputModal);
    if (els.confirmInputBtn) els.confirmInputBtn.addEventListener('click', confirmInputModal);
    if (els.inputModal) {
      els.inputModal.addEventListener('click', (e) => {
        if (e.target === els.inputModal) closeInputModal();
      });
      els.inputModalValue.addEventListener('keydown', (e) => {
    ...
    // 预置位分配模态框
    if (els.closePresetAssignBtn) els.closePresetAssignBtn.addEventListener('click', closePresetAssignModal);
    if (els.presetAssignModal) {
      els.presetAssignModal.addEventListener('click', (e) => {
        if (e.target === els.presetAssignModal) closePresetAssignModal();
      });
    }
