...
(function () {
  ...
  function renderRadioPresets() {
    ...
    const endIdx = Math.min(startIdx + PAGE_SIZE, PRESET_COUNT);
  ...
  function setupEqKnobs() {
    ...
    knobs.forEach(knob => {
      ...
      const valueEl = knob.parentElement.querySelector('.eq-knob-value');
      ...
      function updateKnob(angle) {
        ...
        let newValue = startValue + delta / 3.6;
        newValue = Math.max(-10, Math.min(10, newValue));
        newValue = Math.round(newValue);
        
        const rotation = (newValue + 10) * 3.6 - 90;
        indicator.style.transform = `rotate(${rotation}deg)`;
        valueEl.textContent = (newValue >= 0 ? '+' : '') + newValue + 'dB';
        
        state.eq[band] = newValue;
        updateEqAudio();
      }
      ...
      document.addEventListener('mousemove', (e) => {
        if (dragging) {
          updateKnob(getAngle(e.clientX, e.clientY));
        }
      });
  ...
  function updateEqUI() {
    ['bass', 'mid', 'treble'].forEach(band => {
      const knob = document.querySelector('.eq-knob[data-band="' + band + '"]');
      if (knob) {
        const indicator = knob.querySelector('.eq-knob-indicator');
        const valueEl = knob.parentElement.querySelector('.eq-knob-value');
        const rotation = (state.eq[band] + 10) * 3.6 - 90;
        
        indicator.style.transform = `rotate(${rotation}deg)`;
        valueEl.textContent = (state.eq[band] >= 0 ? '+' : '') + state.eq[band] + 'dB';
      }
    });
  }
  ...
  function bindEvents() {
    ...
    // 预置按钮翻页
    if (els.prevPresetBtn) {
      els.prevPresetBtn.addEventListener('click', () => {
    ...
    if (els.nextPresetBtn) {
      els.nextPresetBtn.addEventListener('click', () => {
        if (state.presetPage.radio < 1) {
          state.presetPage.radio++;
          renderRadioPresets();
        }
      });
