...
(function () {
  ...
  function ensureRadioAudioGraph() {
    ...
    try {
      ...
      // ========== EQ 均衡器 ==========
      A.bassFilter = ctx.createBiquadFilter();
      A.bassFilter.type = 'lowshelf';
      A.bassFilter.frequency.value = 250;
      A.bassFilter.gain.value = 0;

      A.midFilter = ctx.createBiquadFilter();
      A.midFilter.type = 'peaking';
      A.midFilter.frequency.value = 1000;
      A.midFilter.Q.value = 1;
      A.midFilter.gain.value = 0;

      A.trebleFilter = ctx.createBiquadFilter();
      A.trebleFilter.type = 'highshelf';
      A.trebleFilter.frequency.value = 4000;
      A.trebleFilter.gain.value = 0;
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, () => {
            ...
            hlsAudio.play().then(() => {
              ...
              if (state.audio.ctx) {
                ...
                const A = state.audio;
                A.source.connect(A.bassFilter);
                A.bassFilter.connect(A.midFilter);
                A.midFilter.connect(A.trebleFilter);
                A.trebleFilter.connect(A.splitter);
                A.trebleFilter.connect(A.destination);
              }
              startRadioDynamics();
            }).catch(err => {
  ...
  const EQ_PRESETS = {
    ...
    classical: { bass: -2, mid: 2, treble: 5 }
  };
  ...
  function setupEqKnobs() {
    ...
    presetBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (preset) {
          setEq(preset.bass, preset.mid, preset.treble);
        }
      });
    });
  }
  ...
  function updateEqUI() {
    ['bass', 'mid', 'treble'].forEach(band => {
      const fader = document.querySelector('.fader[data-band="' + band + '"]');
      if (fader) {
        const track = fader.querySelector('.fader-track');
        const slider = fader.querySelector('.fader-slider');
        const fill = fader.querySelector('.fader-fill');
  ...
  function updateEqAudio() {
    if (state.audio.ctx) {
      const bassGain = Math.pow(10, state.eq.bass / 20);
      const midGain = Math.pow(10, state.eq.mid / 20);
      const trebleGain = Math.pow(10, state.eq.treble / 20);

      if (state.audio.bassFilter) {
        state.audio.bassFilter.gain.value = bassGain;
      }
      if (state.audio.midFilter) {
        state.audio.midFilter.gain.value = midGain;
      }
      if (state.audio.trebleFilter) {
        state.audio.trebleFilter.gain.value = trebleGain;
      }
    }
  }
