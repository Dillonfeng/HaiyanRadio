...
def fetch_logo_from_page(radio_type, radio_id):
    if radio_type == 'qtfm':
        page_url = f'https://www.qtfm.cn/radios/{radio_id}/'
    else:
        page_url = f'https://www.qingting.fm/radios/{radio_id}/'
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            for pattern in img_patterns:
                ...
                for match in matches:
                    if match and ('logo' in match.lower() or 'cover' in match.lower() or 'avatar' in match.lower()):
                        if match.startswith('//'):
                            match = 'https:' + match
...
def fetch_single_logo(channel, progress):
    ...
    if radio_id:
        log(f'  尝试页面获取 ({radio_type}: {radio_id})')
        logo_url = fetch_logo_from_page(radio_type, radio_id)
