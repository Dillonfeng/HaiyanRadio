...
$logpath = 'd:\Trae Work\RetroRadioDesktop\mobile\build\log_v50_full.txt'
...
Hdr "[2/7] Install v50 APK (push + pm install)"
$installOk = $false
...
Hdr "[4/7] logcat -c, start capture (default buffers only)"
...
Hdr "[5/7] force-stop + cold launch app + 3x BACK to kill modals"
...
Hdr "[6/7] Wait 52s (12s T1 + 28s T2 + 40s final + 12s margin)"
for ($i = 0; $i -lt 52; $i += 4) {
    $sizeKB = 0
    if (Test-Path $logpath) { try { $sizeKB = [math]::Round((Get-Item $logpath).Length / 1KB, 1) } catch {} }
    Inf ("  elapsed " + $i + "s / 52s  logproc alive=" + (-not $logproc.HasExited) + "  logsize=" + $sizeKB + "KB")
